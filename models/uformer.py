...
class MMUformer(nn.Module):
    def __init__(self, img_size=128, in_chans=1, dd_in=1,
        ...
        self.token_type_embedding = nn.Embedding(2, embed_dim // 2)
        ...
        # Input/Output
        self.input_proj_img = InputProj(in_channel=dd_in, out_channel=embed_dim // 2, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.input_proj_sig = InputProj(in_channel=dd_in, out_channel=embed_dim // 2, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)

        self.output_proj_img = OutputProj(in_channel=embed_dim, out_channel=in_chans, kernel_size=3, stride=1)
        self.output_proj_sig = OutputProj(in_channel=embed_dim, out_channel=in_chans, kernel_size=3, stride=1)
    ...
    def forward(self, img, sig):
        ...
        # Output Projection
        y_1 = self.output_proj_img(deconv3[..., :self.embed_dim])
        y_2 = self.output_proj_sig(deconv3[..., self.embed_dim:])
        y_2 = Q @ y_2
        return img + y_1, sig + y_2